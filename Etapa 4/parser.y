...
inicio_programa: inserir_tabela programa
inserir_tabela: /* Vazio */ { pushTabela(&lista_tabelas, tabela_global); }
...
programa: lista 
{
	...
	imprimeTabela(tabela_global);
	destroiListaTabelas(&lista_tabelas);
};
...
definicao_funcao: TK_IDENTIFICADOR '(' lista_parametros ')' TK_OC_MAP tipo bloco_comandos
{
	...
	insereEntradaTabela(&tabela_global, $1);
...
lista_identificadores_globais: TK_IDENTIFICADOR ',' lista_identificadores_globais 	{ strcpy($1->tipo_token,tipo_atual); insereEntradaTabela(&tabela_global, $1);};
lista_identificadores_globais: TK_IDENTIFICADOR						{ strcpy($1->tipo_token,tipo_atual); insereEntradaTabela(&tabela_global, $1);};
